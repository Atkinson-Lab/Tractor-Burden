...
with gzip.open(anc_file, mode='rt') as file:
    ...
    samples = np.array(next(tsv_file))[5:]
    risk_count = np.zeros((anno.shape[0], len(samples)))
    ...
    for line in tsv_file:
        ...
        risk_count[np.array(in_window),] = risk_count[np.array(in_window),] + np.array(line)[5:].astype("int")


selected_genes = np.sum(risk_count, axis = 1) != 0

tbl_anno = anno.loc[selected_genes, ].reset_index(drop=True)
tbl_count = pd.DataFrame(risk_count[selected_genes, ], columns = list(samples), dtype="int")
